...
def get_student(docno: str):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("EXEC usp_Get_Student_By_DocNo @STD_DOCNO=?", (docno,))
...
def update_student(docno: str, student: Student):
    ...
    cursor = conn.cursor()
    student_json = json.dumps(student.model_dump(), default=str)
    cursor.execute("EXEC usp_Update_Student @STD_DOCNO=?, @JSON=?",
                   (docno, student_json))
    conn.commit()
    conn.close()
...
def delete_student(docno: str):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("EXEC usp_Delete_Student @STD_DOCNO=?", (docno,))
    conn.commit()
    conn.close()
